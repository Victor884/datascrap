/*
VALIDACAO FIX_VALORES_V3
Rode este arquivo inteiro no SSMS/Azure Data Studio.

Diferenca do V2:
- V2 corrigiu alguns casos, mas criou divergencias novas em HISTORICO (ex.: 003900843, 010105061, 016107607),
  porque ativou SC.VALOR/SC.JORNADA globalmente em HISTORICO_COMPOSICAO_BASE.
- V3 reverte esse ponto: NAO mexe em FINANCEIRO_BASE nem em HISTORICO_COMPOSICAO_BASE.
- V3 testa apenas COMPOSICAO_SEM_HISTORICO usando PFFINANC atual quando PS_CMP.REF existe em marco/2026.
  Esse foi o ponto que corrigiu os casos PFSALCMP como 012300102 e 012500421 sem mexer nos financeiros/historicos.
...
;WITH
  ...
  (
    ...
    FROM
      (
                      SELECT CHAPA, CRIT = 'SITUACAO'
        FROM CRIT_SITUACAO
      UNION ALL
        SELECT CHAPA, 'FINANCEIRO'
        FROM CRIT_FINANCEIRO
      UNION ALL
        SELECT CHAPA, 'COMPOSICAO'
        FROM CRIT_COMPOSICAO
    ) C
    GROUP BY C.CHAPA
  ),
...
/*
STANDALONE - Rode depois de executar salario_composto_universal_fix_valores_v3_diagnostico.sql
NA MESMA janela/conexao do SSMS/Azure Data Studio.

Por que existe este arquivo:
- #MEU_SALARIO_COMPOSTO e tabela temporaria e sobrevive entre batches na mesma conexao.
- Variaveis DECLARE nao sobrevivem entre execucoes/batches.
- Portanto este diagnostico redeclara @CODCOLIGADA/@DATA_BASE/@COMPETENCIA/etc.

Ordem:
1) Execute salario_composto_universal_fix_valores_v3_diagnostico.sql inteiro.
2) Sem fechar a janela/conexao, execute este arquivo inteiro.
*/
...
IF OBJECT_ID('tempdb..#MEU_SALARIO_COMPOSTO') IS NULL
BEGIN
  RAISERROR('Temp table #MEU_SALARIO_COMPOSTO nao existe. Rode primeiro salario_composto_universal_fix_valores_v3_diagnostico.sql na mesma conexao.', 16, 1);
  RETURN;
END;

IF OBJECT_ID('tempdb..#SCFIN_IMPACTO') IS NOT NULL DROP TABLE #SCFIN_IMPACTO;
...
SELECT
  IMPACTO_ATIVAR_SCFIN,
  QTD = COUNT(*)
FROM #SCFIN_IMPACTO
WHERE O_VALOR_MENSAL IS NULL
  OR MATCH_MEU = 0
  OR (MATCH_MEU = 1 AND SC_RECMODIFIEDON IS NOT NULL AND MATCH_SC = 0)
GROUP BY IMPACTO_ATIVAR_SCFIN
ORDER BY QTD DESC;
